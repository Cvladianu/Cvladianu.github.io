...
q("writeEmbed",function(){var a=F("PLAYER_CONFIG",void 0);"1"!=a.privembed&&lk();"gvn-experiment"==a.args.ps&&(document.body.style.backgroundColor="transparent");var b=document.referrer,c=F("POST_MESSAGE_ORIGIN");window!=window.top&&b&&b!=document.URL&&(a.args.loaderUrl=b);F("LIGHTWEIGHT_AUTOPLAY")&&(a.args.autoplay="1");a.args.autoplay&&qj(a.args);Vk=b=fh("player",a);b.addEventListener("onScreenChanged",$k);b.addEventListener("onLogClientVeCreated",Zk);b.addEventListener("onLogServerVeCreated",al);
b.addEventListener("onLogVeClicked",bl);b.addEventListener("onLogVesShown",cl);b.addEventListener("onReady",dl);var d=F("POST_MESSAGE_ID","player");F("ENABLE_JS_API")?Xk=new Lj(b):F("ENABLE_POST_API")&&v(d)&&v(c)&&(Wk=new Oj(window.parent,d,c),Xk=new Dj(b,Wk.g));F("ENABLE_CAST_API")||(a.args.enablecastapi="0");F("BG_P")&&(F("BG_I")||F("BG_IU"))&&fc();nd();rj=b;rj.addEventListener("SUBSCRIBE",uj);rj.addEventListener("UNSUBSCRIBE",xj);sj.push(R(Bh,yj),R(Gh,zj))},void 0);
